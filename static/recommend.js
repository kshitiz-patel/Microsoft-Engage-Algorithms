$(function () {
  ...
  $('.movie-button').on('click', function () {
    var my_api_key = '599f5fa015e623cb1219b925d16b4a69';
    var title = $('.movie').val();
    if (title == "") {
...
// will give a random movie based on the number it generates
function randomMovie(min, max) {
  var my_api_key = '599f5fa015e623cb1219b925d16b4a69';
  var randonNumber = Math.floor(Math.random() * (max - min)) + min;
  var movie = movies[randonNumber];
  load_details(my_api_key, movie);
}

// used to invoke the enter key for search button
...
function show_details(movie_details, arr, movie_title, my_api_key, movie_id) {
  ...
  if (runtime % 60 == 0) {
    ...
    runtime = Math.floor(runtime / 60) + " hr " + (runtime % 60) + " min"
  }
